...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "pointer-events-none absolute inset-x-0 inset-y-0 bg-bg transition-opacity duration-500",
      ...
      >
        <a href="#top" className="flex items-center gap-2.5" onClick={(e) => { e.preventDefault(); handleNav("#top"); }}>
          <span className="flex h-9 w-9 items-center justify-center rounded-full border border-primary/40 bg-primary/10">
            <span className="h-2 w-2 rounded-full bg-primary shadow-[0_0_12px_2px_rgba(0,87,184,0.5)]" />
          </span>
        ...
        <div className="hidden lg:block">
          <a
            ...
            className="group inline-flex items-center gap-2 rounded-full border border-primary/40 bg-primary/10 px-5 py-2.5 font-mono text-xs uppercase tracking-[0.2em] text-primary transition-all hover:bg-primary hover:text-white"
          >
            {company.phone}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="mx-4 mt-3 flex flex-col gap-1 rounded-3xl border border-border bg-white/95 p-4 shadow-[0_20px_60px_-15px_rgba(17,24,39,0.15)] backdrop-blur-2xl lg:hidden"
          >
            {links.map((link) => (
              <button
                key={link.href}
